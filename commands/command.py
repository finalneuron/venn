...
class CmdAbilities(Command):
    ...
    key = "abilities"
    aliases = ["abi", "stats"]
    locks = "cmd:all()"         # anyone can use it
    help_category = "General"

    def func(self):
        caller = self.caller
        strength = caller.db.strength or 0
        agility = caller.db.agility or 0
        magic = caller.db.magic or 0
        level = caller.db.level or 1

        caller.msg(
            f"|wLevel:|n {level}\n"
            f"|wSTR:|n {strength}  |wAGI:|n {agility}  |wMAG:|n {magic}"
        )
